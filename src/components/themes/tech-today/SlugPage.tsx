...
function PageContent({ pageId, editorContent }: { pageId: string, editorContent: string }) {
    ...
    if (isLoadingAreas || isLoadingWidgets) {
        return <div className="prose prose-invert lg:prose-lg max-w-none mx-auto"><p>Loading content...</p></div>
    }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-12">
            <div className="lg:col-span-3">
                ...
                <header className="mb-8">
                    <h1 className="text-4xl font-extrabold font-headline tracking-tight lg:text-6xl mb-4 text-cyan-300">{item.title}</h1>
                    ...
                    <footer className="mt-12">
                        <div className="flex flex-wrap gap-2">
                            {(item as Post).tagIds!.map(tag => (
                                <Badge key={tag} variant="outline" className="border-cyan-400/50 text-cyan-400">{tag}</Badge>
                            ))}
                        </div>
                    </footer>
                )}
